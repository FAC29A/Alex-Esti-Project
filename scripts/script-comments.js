...
// create a variable with Object which is empty, later we add each crime category
const crimeLayers = {};

// *************************************************************Load HTML***************************************************************
// every time we load the page doing this:
//                          action          ,   what happens
...
// ********************************************Initialize map***********************************************************************
//*************************************every load or reload calling this funcion
function initializeMap() {
  // Initialize the map at the beginning
  //map = L.map("map").setView([latitude, longitude], zoomLevel);
  // L (the API of Leaflet) will take and use the map from Leaflet
  map = L.map(mapElement, { zoomControl: true }).setView(
    [latitude, longitude],
    zoomLevel //the zoom in or our from the map
  );

  //******************* */ Initialize a layer group for each crime category
  //crimes taken from data.js file
  crimes.forEach((crime) => {
    //creating the crimes layers,   Leaflet method layerGroup.
    crimeLayers[crime.url] = L.layerGroup().addTo(map);
  });

  //******************************************* */ creating layers on the map with the small icon on the top right screen
  var overlayMaps = {};

  // Construct overlayMaps without "all-crime" and use the crime's name instead of its URL
  //we are using the keys from crimeLayers which taken from Data.js
  Object.keys(crimeLayers).forEach((key) => {
    if (key !== "all-crime") {
      //                            c is a method
      const crimeData = crimes.find((c) => c.url === key);
      //will define the crimes from its names in data.js
      //if crimeData and crimeData name are exist:
  ...
  // put the crimes categories as a layers on the icon on the right top
  ...
  //we calling this function and we using latitude and longtitude
...
async function getPostcodeCoordinates(postcode) {
  ...
  try {
    ...
    if (response.status === 200 && data.result) {
      ...
      console.log("Server Error", data.error);
      alert(
        "There was an error fetching the postcode information. Please try again later."
      );
    }
  } catch (error) {
    console.log("Fetch Error", error);
    alert(
      "There was an unexpected error. Please check your internet connection and try again."
    );
  }
}

//Draw regions using latitude and longiute
//will get data from the API and store it here in the promise
async function fetchAndDrawBoundaryCoordinates(myLatitude, myLongitude) {
  return new Promise(async (resolve, reject) => {
    ...
    //the URL will return what is here inside - we store here the URL
    ...
    try {
      //create fetch to the top URL. await - wait for the information from fetch
      const response = await fetch(forceAndNeighbourhoodUrl);
      //                         transfrom from json object the data from the police URL
      const data = await response.json();

      //status - we check the status property from the response json object.
      // we are checking if the fetch was right . 200 is the right from all errors code and also data is right
      if (response.status === 200 && data) {
        //we are definding the place of the API as the police force and neighbourhood
        forceId = data.force;
        neighbourhoodId = data.neighbourhood;

        // Check if the neighbourhood is the same as before
        //will stop spinning if you stop press the mouse
        if (neighbourhoodId === currentNeighbourhoodId) {
          crosshair.classList.remove("spin");
          return; // Return early without redrawing
        }
        // Update the currentNeighbourhoodId
        // the neighberhood we got from the fetch
...
//taken from the map API and shows on the right top icon
...
//taking the 2 osm (types of maps)
var baseMaps = {
  OpenStreetMap: osm,
